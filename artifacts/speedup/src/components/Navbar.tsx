              ...
              <nav className="flex-1 px-8 space-y-1 overflow-y-auto">
                {NAV_LINKS.map((link, i) => (
                  <motion.button
                    ...
                    onClick={() => go(link.href)}
                    className="w-full text-left py-4 text-2xl font-black uppercase transition-colors border-b border-white/15 tracking-tight"
                    style={{ color: isActive(link.href) ? "rgba(255,255,255,0.5)" : "white" }}
                  >
                    {link.label}
                  </motion.button>
                ))}
